...
body=$(jq -cn --arg s "$sys" --arg q "$task" --argjson n "$maxtok" \
    '{messages:[{role:"system",content:$s},{role:"user",content:$q}],max_tokens:$n,temperature:0.3,chat_template_kwargs:{enable_thinking:false}}')
...
{
    printf '<!-- PROPOSAL — drafted by the tend %s node on %s.\n' "$name" "$(date '+%Y-%m-%d %H:%M')"
